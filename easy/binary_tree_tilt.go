...
// is similar if there the node does not have a right child.
//
//
//
// Example 1:
...
// Tilt of node 1 : |2-3| = 1 (left subtree is just left child, so sum is 2; right subtree is
// just right child, so sum is 3)
...
// Tilt of node 2 : |3-5| = 2 (left subtree is just left child, so sum is 3; right subtree is
// just right child, so sum is 5)
...
func tiltValue(root *TreeNode, tilt *int) int {
	if root == nil {
		return 0
	}

	l := tiltValue(root.Left, tilt)
	r := tiltValue(root.Right, tilt)

	abs := int(math.Abs(float64(l - r)))
	*tilt += abs
