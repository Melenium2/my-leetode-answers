package easy

import (
	"sort"
	"strconv"
)

// You are given an integer array score of size n, where score[i] is the
// score of the ith athlete in a competition. All the scores are guaranteed to be unique.
//
// The athletes are placed based on their scores, where the 1st place athlete has
// the highest score, the 2nd place athlete has the 2nd highest score, and so on.
// The placement of each athlete determines their rank:
//
// The 1st place athlete's rank is "Gold Medal".
// The 2nd place athlete's rank is "Silver Medal".
// The 3rd place athlete's rank is "Bronze Medal".
// For the 4th place to the nth place athlete, their rank is their placement number
// (i.e., the xth place athlete's rank is "x").
// Return an array answer of size n where answer[i] is the rank of the ith athlete.
//
//
//
// Example 1:
// Input: score = [5,4,3,2,1]
// Output: ["Gold Medal","Silver Medal","Bronze Medal","4","5"]
// Explanation: The placements are [1st, 2nd, 3rd, 4th, 5th].
//
// Example 2:
// Input: score = [10,3,8,9,4]
// Output: ["Gold Medal","5","Bronze Medal","Silver Medal","4"]
// Explanation: The placements are [1st, 5th, 3rd, 2nd, 4th].
func findRelativeRanks(score []int) []string {
	l := len(score)
	p := []string{"Gold Medal", "Silver Medal", "Bronze Medal"}
	m := make(map[int]int, l)
	r := make([]string, l)
	for i, s := range score {
		m[s] = i
	}
	sort.Ints(score)
	for c, s := range score {
		ix := l - c - 1
		if ix <= 2 {
			r[m[s]] = p[ix]
		} else {
			r[m[s]] = strconv.Itoa(ix + 1)
		}
	}
	return r
}

type PriorityElement struct {
	data     int
	priority int
	next     *PriorityElement
}

type priorityQueue struct {
	head *PriorityElement
}

func newPriorityQueue() *priorityQueue {
	return &priorityQueue{}
}

func (pq *priorityQueue) newHead(data, priority int) *PriorityElement {
	return &PriorityElement{
		data:     data,
		priority: priority,
		next:     nil,
	}
}

func (pq *priorityQueue) push(data, priority int) {
	elem := pq.newHead(data, priority)

	if pq.head == nil {
		pq.head = elem
		return
	}

	start := pq.head

	if pq.head.priority < priority {
		elem.next = pq.head
		pq.head = elem
	} else {
		for start.next != nil && start.next.priority > priority {
			start = start.next
		}

		elem.next = start.next
		start.next = elem
	}
}

func (pq *priorityQueue) pop() int {
	temp := pq.head
	pq.head = pq.head.next
	return temp.data
}

func (pq *priorityQueue) isEmpty() bool {
	return pq.head == nil
}

// Go solution
//	l := len(score)
//	p := []string{"Gold Medal", "Silver Medal", "Bronze Medal"}
//	m := make(map[int]int, l)
//	r := make([]string, l)
//	for i, s := range score {
//		m[s] = i
//	}
//	sort.Ints(score)
//	for c, s := range score {
//		ix := l - c - 1
//		if ix <= 2 {
//			r[m[s]] = p[ix]
//		} else {
//			r[m[s]] = strconv.Itoa(ix + 1)
//		}
//	}
//	return r
