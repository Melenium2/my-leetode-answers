...
func combinationSum(candidates []int, target int) [][]int {
	...
	findCombination(0, candidates, &comb, 0, target, &res)

	return res
}

func findCombination(i int, candidates []int, combinations *[]int, total, target int, res *[][]int) {
	if total == target {
		c := make([]int, len(*combinations))
		copy(c, *combinations)
	...
	*combinations = append(*combinations, candidates[i])
	findCombination(i, candidates, combinations, total+candidates[i], target, res)
	*combinations = (*combinations)[:len(*combinations)-1]
	findCombination(i+1, candidates, combinations, total, target, res)
}
