...
func totalFruit(fruits []int) int {
	...
	var (
		set        = make(map[int]int, 3)
		startIndex = 0
		maxValues  = -1
	)

	for endIndex := 0; endIndex < len(fruits); endIndex++ {
		curr := fruits[endIndex]
		set[curr]++

		for len(set) > 2 {
			startWindowValue := fruits[startIndex]

			set[startWindowValue]--

			if v := set[startWindowValue]; v == 0 {
				delete(set, startWindowValue)
			}

			startIndex++
		}

		maxValues = max(maxValues, endIndex-startIndex+1)
	}
